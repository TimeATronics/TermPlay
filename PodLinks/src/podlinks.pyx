...
# PodLinks: Simple utility to parse RSS feeds and save them to txt files
# Author: TimeATronics
...
def checkURL(url):
    # Check if requested url is responsive
    try:
...
def feedinput():
    feedurl=input("Enter URL of desired RSS feed:\n>>> ")
    if checkURL(feedurl):
        return feedurl
    else:
        return 1
...
def createdir(name):
    try:
        ...
        sys.exit(1)
# https links might not always work
# replace https with http in those cases
def main():
    try:
        feedurl=feedinput()
        if feedurl == 1:
            print("Given RSS Feed does not exist")
            sys.exit(1)
        else:
            parsed = parse(feedurl, urllib.request.urlopen(feedurl))
            dirname = input("Enter name of directory to save file:\n>>> ")
            if dirname[-1] != "/":
                dirname += "/"
            createdir(dirname)
            filename = input("Enter name of file to save text to:\n>>> ")
            file = open(str(dirname+filename), "a")
